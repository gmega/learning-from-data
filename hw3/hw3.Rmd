...
Generalization theory dictates that if some learning model has a breakpoint, then its growth function is bounded by a polynomial. A breakpoint, in turn, is the largest value of $N$ for which the growth function equals the upper bound, $2^N$. This imply that valid growth functions must either:

* be equal to $2^N$ for all $N$; or; 
* be polynomials of $N$ which nevertheless must be always smaller or equal to $2^N$ for all positive integers (i.e., for function $f$, we must have that $f_{i}(N)$ that $f_{i}(N) \leq 2^N$ for all positive integers).
...
**Function (iv).** This is also trivially not a valid growth function, as it is not polynomial in $N$, but at the same time it is smaller than $2^N$ for $N \geq 1$ ($\lfloor N/2 \rfloor < N$ for $N > 2$)
...
```{r 2-intervals, fig.align='center', out.width='500pt', fig.cap='Non-overlapping intervals.'}
...
Assume now, without loss of generality, that we introduced an $8^{th}$ point between the two adjacent red points $x_1$ and $x_2$. We will demonstrate that if $x_1$ and $x_2$ are red, then either _i)_ $x_3$ is also red, or _ii)_ $x_3$ is not a part of a the convex hull. Either way, an $8$-point set cannot be shattered with a triangle.
